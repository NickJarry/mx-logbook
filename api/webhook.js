...
export default async function handler(req, res) {
  ...
  // Plan hierarchy for highest_plan tracking
  const PLAN_ORDER = ['pro','shop','proshop','enterprise'];
  function isPlanHigher(newPlan, currentHighest) {
    return PLAN_ORDER.indexOf(newPlan) > PLAN_ORDER.indexOf(currentHighest || 'pro');
  }

  async function updateHighestPlan(email, newPlan) {
    if (!newPlan || !PLAN_ORDER.includes(newPlan)) return;
    const profiles = await fetch(`${SUPABASE_URL}/rest/v1/profiles?email=eq.${encodeURIComponent(email)}&select=highest_plan`, {
      headers: { 'apikey': SUPABASE_SERVICE_KEY, 'Authorization': `Bearer ${SUPABASE_SERVICE_KEY}` }
    }).then(r => r.json());
    const current = profiles[0]?.highest_plan;
    if (isPlanHigher(newPlan, current)) {
      await fetch(`${SUPABASE_URL}/rest/v1/profiles?email=eq.${encodeURIComponent(email)}`, {
        method: 'PATCH',
        headers: { 'apikey': SUPABASE_SERVICE_KEY, 'Authorization': `Bearer ${SUPABASE_SERVICE_KEY}`, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
        body: JSON.stringify({ highest_plan: newPlan })
      });
      console.log(`Updated highest_plan for ${email}: ${newPlan}`);
    }
  }
  ...
  try {
    ...
    switch (event.type) {
      ...
      case 'customer.subscription.created': {
        ...
        if (plan) await updateHighestPlan(email, plan);
      ...
      case 'customer.subscription.updated': {
        ...
        if (status === 'active') {
          ...
          await updateHighestPlan(email, plan);
      ...
      case 'invoice.paid': {
        const email = obj.customer_email || await getCustomerEmail(obj.customer);
        if (email) {
          // Only set active if this is a real charge — $0 trial invoice must not overwrite trialing
          if (obj.amount_paid > 0) {
            await updateProfile(email, { subscription_status: 'active' });
            console.log(`Payment succeeded for ${email}`);
            // Mark the first paid invoice date on referral_rewards for this referred user
            try {
              const profile = await fetch(`${SUPABASE_URL}/rest/v1/profiles?email=eq.${encodeURIComponent(email)}&select=id`, {
                headers: { 'apikey': SUPABASE_SERVICE_KEY, 'Authorization': `Bearer ${SUPABASE_SERVICE_KEY}` }
              }).then(r => r.json());
              const uid = profile[0]?.id;
              if (uid) {
                // Set paid_at only if not already set (first payment only)
                await fetch(`${SUPABASE_URL}/rest/v1/referral_rewards?referred_id=eq.${uid}&status=eq.pending&paid_at=is.null`, {
                  method: 'PATCH',
                  headers: { 'apikey': SUPABASE_SERVICE_KEY, 'Authorization': `Bearer ${SUPABASE_SERVICE_KEY}`, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
                  body: JSON.stringify({ paid_at: new Date().toISOString() })
                });
              }
            } catch(refErr) { console.log('Referral paid_at error:', refErr.message); }
          } else {
            console.log(`Trial invoice ($0) for ${email} — preserving trialing status`);
          }
        }
