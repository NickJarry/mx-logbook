  ...
  try {
    ...
    switch (event.type) {
      ...
      case 'invoice.paid': {
        const email = obj.customer_email || await getCustomerEmail(obj.customer);
        if (email) {
          // Only set active if this is a real charge — don't overwrite trialing on $0 trial invoice
          if (obj.amount_paid > 0) {
            await updateProfile(email, { subscription_status: 'active' });
            console.log(`Payment succeeded for ${email}`);
          } else {
            console.log(`Trial invoice paid ($0) for ${email} — preserving trialing status`);
          }
        }
