...
export default async function handler(req, res) {
  ...
  try {
    ...
    switch (event.type) {
      ...
      // New subscription created — check if card fingerprint was used before
      ...
      // Trial ending in 3 days — flag it and send reminder email
      case 'customer.subscription.trial_will_end': {
        const email = await getCustomerEmail(obj.customer);
        if (email) {
          await updateProfile(email, { trial_ending_soon: true });
          // Send trial ending email
          const trialEnd = obj.trial_end ? new Date(obj.trial_end * 1000).toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'}) : 'soon';
          await fetch(`${process.env.VERCEL_URL || 'https://mx-logbook.com'}/api/generate`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ type: 'send_trial_ending', email, trial_end_date: trialEnd })
          }).catch(() => {});
