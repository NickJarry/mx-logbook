export default async function handler(req, res) {
  ...
  try {
    ...
    const pending = await sbFetch(
      `/referral_rewards?status=eq.pending&paid_at=lte.${cutoff}&paid_at=not.is.null&select=id,referrer_id,referred_id,referrer_plan,referred_plan,reward_value`
    );
    console.log('Referral rewards check -- cutoff:', cutoff, 'pending count:', Array.isArray(pending) ? pending.length : 'error', JSON.stringify(pending).substring(0,200));
