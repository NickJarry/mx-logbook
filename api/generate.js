export default async function handler(req, res) {
  ...
  return res.status(200).json({ user: data.user, session: data, profile: { ...profile, subscription_status: 'trialing' } });
    ...
    if (type === 'invite_member') {
      ...
      // Expire any existing pending invites for this email+company
    ...
    if (type === 'get_invite') {
      ...
      const invites = await sbFetch(`/mechanic_invites?token=eq.${token}&select=*`);
    ...
    if (type === 'check_access') {
      ...
      let profiles;
      if (user_id) {
        profiles = await sbFetch(`/profiles?id=eq.${user_id}&select=plan,subscription_status,cancelled_at,created_at,full_name,cert_number,company_id,role`);
      } else {
        profiles = await sbFetch(`/profiles?email=eq.${encodeURIComponent(email)}&select=plan,subscription_status,cancelled_at,created_at,full_name,cert_number,company_id,role`);
      }
