export default async function handler(req, res) {
  ...
  return res.status(200).json({ user: data.user, session: data, profile: { ...profile, subscription_status: 'trialing' } });
    ...
    if (type === 'save_tech_info') {
      ...
      await sbFetch(`/profiles?id=eq.${user_id}`, {
        ...
        body: JSON.stringify({ tech_name: tech_name || null, cert_num: cert_num || null })
      });
      return res.status(200).json({ success: true });
    }
    ...
    if (type === 'check_access') {
      ...
      let profiles;
      if (user_id) {
        profiles = await sbFetch(`/profiles?id=eq.${user_id}&select=plan,subscription_status,cancelled_at,created_at,tech_name,cert_num`);
      } else {
        profiles = await sbFetch(`/profiles?email=eq.${encodeURIComponent(email)}&select=plan,subscription_status,cancelled_at,created_at,tech_name,cert_num`);
      }
  ...
  if (ageSeconds < 30) {
    return res.status(200).json({ blocked: false, plan: profile.plan, subscription_status: 'trialing', tech_name: profile.tech_name || null, cert_num: profile.cert_num || null });
  }
  return res.status(200).json({ blocked: true, message: 'Payment not completed. Please complete your signup at mx-logbook.com to activate your account.' });
      ...
      return res.status(200).json({ blocked: false, plan: profile.plan, subscription_status: profile.subscription_status || null, tech_name: profile.tech_name || null, cert_num: profile.cert_num || null });
    }
