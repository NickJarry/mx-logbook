export default async function handler(req, res) {
  ...
  return res.status(200).json({ user: data.user, session: data, profile: { ...profile, subscription_status: 'trialing' } });
    ...
    if (type === 'update_member_role') {
      ...
      if (!member_id || !role) return res.status(400).json({ error: 'Missing required fields.' });
      // Only admins can update roles - verify caller is admin
      const callerProfile = await fetch(`${process.env.SUPABASE_URL}/rest/v1/profiles?id=eq.${user_id}&select=role,company_id`, {
        headers: { 'apikey': process.env.SUPABASE_SERVICE_KEY, 'Authorization': `Bearer ${process.env.SUPABASE_SERVICE_KEY}` }
      }).then(r => r.json());
      if (callerProfile[0]?.role !== 'admin' && callerProfile[0]?.role !== null) {
        return res.status(403).json({ error: 'Only admins can change roles.' });
      }
      await fetch(`${process.env.SUPABASE_URL}/rest/v1/profiles?id=eq.${member_id}`, {
        method: 'PATCH',
        headers: { 'apikey': process.env.SUPABASE_SERVICE_KEY, 'Authorization': `Bearer ${process.env.SUPABASE_SERVICE_KEY}`, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
        body: JSON.stringify({ role, plan: role })
      });
      return res.status(200).json({ success: true });
    }
