...
if (!company_id) return res.status(400).json({ error: 'Account setup is still in progress. Please log out, log back in, and try again.' });
  ...
  return res.status(200).json({ blocked: true, message: 'Payment not completed. Please complete your signup at mx-logbook.com to activate your account.' });
    ...
    if (type === 'change_plan') {
      ...
      const planLabels = { pro:'Pro ($9.99/mo)', shop:'Shop ($299/mo)', proshop:'Shop Pro ($599/mo)', enterprise:'Enterprise ($999/mo)' };
      await sendEmail({
        to: customer_email,
...
if (type === 'release_running_report') {
      const { report_id, polished_text } = req.body;
      if (!user_id || !report_id) return res.status(400).json({ error: 'Missing params.' });
      const updatePayload = { status: 'released', is_saved: true, expires_at: null, released_at: new Date().toISOString() };
      if (polished_text) updatePayload.polished_text = polished_text;
    ...
    if (type === 'get_released_reports') {
      ...
      const resp = await fetch(`${process.env.SUPABASE_URL}/rest/v1/running_reports?user_id=eq.${user_id}&status=eq.released&order=released_at.desc&limit=100&select=id,tail_number,aircraft,entries,polished_text,released_at,updated_at`, {
        headers: {
          'apikey': process.env.SUPABASE_SERVICE_KEY,
          'Authorization': `Bearer ${process.env.SUPABASE_SERVICE_KEY}`
        }
