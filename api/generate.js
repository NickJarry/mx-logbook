export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === 'auth_login') {
      ...
      // No profile found after retries — account was deleted or never created
      if (!profile) {
    ...
    if (type === 'change_plan') {
      ...
      const searchData = await searchResp.json();
      const customer = searchData.data?.[0];
      if (!customer) return res.status(400).json({ error: 'No billing account found. Please contact hello@mx-logbook.com.' });
