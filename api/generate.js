export default async function handler(req, res) {
  ...
  return res.status(200).json({ blocked: true, message: 'Payment not completed. Please complete your signup at mx-logbook.com to activate your account.' });
    ...
    if (type === 'get_company_plan') {
      const { company_id } = req.body;
      if (!company_id) return res.status(200).json({ plan: null });
      const resp = await fetch(`${process.env.SUPABASE_URL}/rest/v1/companies?id=eq.${company_id}&select=plan`, {
        headers: { 'apikey': process.env.SUPABASE_SERVICE_KEY, 'Authorization': `Bearer ${process.env.SUPABASE_SERVICE_KEY}` }
      });
      const data = await resp.json();
      return res.status(200).json({ plan: data[0]?.plan || null });
    }
