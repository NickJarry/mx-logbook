  ...
  try {
    ...
    if (type === 'auth_signup') {
      ...
      if (uid) {
        ...
        // Try inserting profile up to 3 times
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
          try {
            ...
            if (result && !result.error && (Array.isArray(result) ? result[0] : result)?.id) {
              profileInserted = true;
              console.log('Profile inserted on attempt', attempt + 1);
              break;
            }
            console.log('Profile insert attempt', attempt + 1, 'result:', JSON.stringify(result));
          } catch(e) {
            console.log('Profile insert attempt', attempt + 1, 'error:', e.message);
          }
          if (attempt < 2) await new Promise(r => setTimeout(r, 1000));
        }

        if (!profileInserted) {
          console.error('Profile insert failed after 3 attempts for', email);
          // Alert owner
          sendEmail({
            to: 'hello@mx-logbook.com',
            subject: '[MX-Logbook Alert] Profile insert failed',
            html: '<p>Profile insert failed for ' + email + ' (uid: ' + uid + '). Manual intervention required.</p>'
          }).catch(()=>{});
        }
    ...
    if (type === 'auth_login') {
      ...
      // Retry profile fetch up to 3 times — handles brief insert delay after signup
      let profile = null;
      for (let attempt = 0; attempt < 3; attempt++) {
      ...
      // No profile found — try to create it as a fallback (handles failed signup inserts)
      if (!profile) {
        console.log('No profile found on login for', data.user?.id, '- attempting recovery insert');
        ...
        try {
          ...
          console.log('Recovery insert result:', JSON.stringify(profile));
        } catch(e) {
          console.error('Recovery insert failed:', e.message);
        }
      }

      // Still no profile — something is deeply wrong
      if (!profile || profile.error) {
        return res.status(403).json({ error: 'Account setup incomplete. Please contact hello@mx-logbook.com.' });
      }
    ...
    if (type === 'check_access') {
      ...
      let profiles;
      if (user_id) {
        profiles = await sbFetch(`/profiles?id=eq.${user_id}&select=plan,subscription_status,cancelled_at`);
      } else {
        profiles = await sbFetch(`/profiles?email=eq.${encodeURIComponent(email)}&select=plan,subscription_status,cancelled_at`);
      }
    ...
    if (type === 'change_plan') {
      ...
      const planLabels = { solo:'Solo ($9.99/mo)', pro:'Pro ($16.99/mo)', shop:'Shop ($149/mo)', team:'Team ($299/mo)', proshop:'Pro Shop ($499/mo)', enterprise:'Enterprise ($899/mo)' };
      await sendEmail({
        to: customer_email,
