export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === 'auth_signup') {
      const { email, password, plan, referral_code, promo_signup } = req.body;
      ...
      if (uid) {
        ...
        // Promo accounts are active immediately, no Stripe required
        const initialStatus = promo_signup ? 'active' : 'pending_payment';
  ...
  return res.status(200).json({ user: data.user, session: data, profile: { ...profile, subscription_status: 'trialing' } });
    ...
    if (type === 'check_access') {
      ...
      let profiles;
      if (user_id) {
        profiles = await sbFetch(`/profiles?id=eq.${user_id}&select=plan,subscription_status,cancelled_at,created_at`);
      } else {
        profiles = await sbFetch(`/profiles?email=eq.${encodeURIComponent(email)}&select=plan,subscription_status,cancelled_at,created_at`);
      }
  ...
  return res.status(200).json({ blocked: true, message: 'Payment not completed. Please complete your signup at mx-logbook.com to activate your account.' });
      ...
      return res.status(200).json({ blocked: false, plan: profile.plan, subscription_status: profile.subscription_status || null });
    }
    ...
    if (type === 'change_plan') {
      ...
      const planLabels = { solo:'Solo ($9.99/mo)', pro:'Pro ($19.99/mo)', shop:'Shop ($149/mo)', team:'Team ($299/mo)', proshop:'Pro Shop ($499/mo)', enterprise:'Enterprise ($899/mo)' };
      await sendEmail({
        to: customer_email,
