export default async function handler(req, res) {
  ...
  return res.status(200).json({ blocked: true, message: 'Payment not completed. Please complete your signup at mx-logbook.com to activate your account.' });
    ...
    if (type === 'get_company_aog') {
      // Get all active AOG sessions for a company
      const { company_id } = req.body;
      if (!company_id) return res.status(200).json({ sessions: [] });
      const team = await sbFetch(`/profiles?company_id=eq.${company_id}&select=id,email,full_name`);
      const memberMap = {};
      (Array.isArray(team) ? team : []).forEach(m => { memberMap[m.id] = { email: m.email, name: m.full_name || m.email }; });
      const sessions = await sbFetch(`/aog_sessions?company_id=eq.${company_id}&status=eq.active&order=updated_at.desc`);
      const enriched = (Array.isArray(sessions) ? sessions : []).map(s => ({
        ...s,
        mechanic_name: memberMap[s.created_by]?.name || memberMap[s.created_by]?.email || 'Unknown'
      }));
      return res.status(200).json({ sessions: enriched });
    }

    if (type === 'create_aog_session') {
      const { company_id, tail_number, aircraft, title } = req.body;
      if (!user_id || !company_id) return res.status(400).json({ error: 'Missing params.' });
      const session = await sbFetch('/aog_sessions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Prefer': 'return=representation' },
        body: JSON.stringify({
          company_id, tail_number: tail_number || '', aircraft: aircraft || '',
          title: title || `AOG - ${tail_number || 'Unknown'}`,
          status: 'active', created_by: user_id,
          entries: JSON.stringify([]), updated_at: new Date().toISOString()
        })
      });
      return res.status(200).json({ session: Array.isArray(session) ? session[0] : session });
    }

    if (type === 'add_aog_entry') {
      const { session_id, entry_text, tech_name, cert_num } = req.body;
      if (!session_id || !entry_text) return res.status(400).json({ error: 'Missing params.' });
      // Get current entries
      const sessions = await sbFetch(`/aog_sessions?id=eq.${session_id}&select=entries`);
      ...
      const newEntry = {
        id: Date.now(),
        time: new Date().toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' }) + 'L',
        text: entry_text,
      ...
      await sbFetch(`/aog_sessions?id=eq.${session_id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
    ...
    if (type === 'get_aog_session') {
      const { session_id } = req.body;
      if (!session_id) return res.status(400).json({ error: 'Missing session_id.' });
      const sessions = await sbFetch(`/aog_sessions?id=eq.${session_id}&select=*`);
      return res.status(200).json({ session: sessions[0] || null });
    }
    ...
    if (type === 'close_aog_session') {
      ...
      await sbFetch(`/aog_sessions?id=eq.${session_id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
        body: JSON.stringify({ status: 'resolved', closed_at: new Date().toISOString() })
      });
      return res.status(200).json({ success: true });
    }
